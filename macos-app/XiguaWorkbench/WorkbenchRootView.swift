        ...
        .overlay(alignment: .bottomTrailing) {
            if let notice = store.notice {
                Text(notice)
                    ...
                    .background {
                        Rectangle().fill(WorkbenchTheme.ink).offset(x: 6, y: 6)
                    }
                    .padding(22)
                    .task(id: notice) {
        ...
        VStack(alignment: .leading, spacing: 0) {
            ...
            VStack(spacing: 7) {
                ...
                Button(action: openSettings) {
                    SidebarRow(number: 7, title: "设置", trailing: "本地", isSelected: false)
                }
                .buttonStyle(.plain)
            }
            .padding(.top, 54)
...
private struct WatermelonBrandMark: View {
    var body: some View {
        ZStack {
            Circle().fill(WorkbenchTheme.ink).offset(x: 4, y: 4)
